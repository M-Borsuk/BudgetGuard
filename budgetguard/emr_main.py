...
def run(task: str, partition_id: str):
    if task == "ingest_account_data":
        ...
        from budgetguard.core.pipelines.bronze_to_silver.balances_pipeline import BronzeToSilverBalancesPipeline

        pipeline = BronzeToSilverBalancesPipeline(partition_id)
    elif task == "bronze_to_silver_details":
        from budgetguard.core.pipelines.bronze_to_silver.details_pipeline import BronzeToSilverDetailsPipeline

        pipeline = BronzeToSilverDetailsPipeline(partition_id)
    elif task == "bronze_to_silver_transactions":
        from budgetguard.core.pipelines.bronze_to_silver.transactions_pipeline import BronzeToSilverTransactionsPipeline

        pipeline = BronzeToSilverTransactionsPipeline(partition_id)
    elif task == "bronze_to_silver_metadata":
        from budgetguard.core.pipelines.bronze_to_silver.metadata_pipeline import BronzeToSilverMetadataPipeline
