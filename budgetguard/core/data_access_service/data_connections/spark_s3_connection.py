...
class SparkS3Connection(Connection):
    ...
    def __create_spark_session__(self) -> SparkSession:
        ...
        aws_credentials = self.s3_connection.get_aws_secret("budget_guard_aws_credentials")
        spark._jsc.hadoopConfiguration().set("fs.s3a.access.key", aws_credentials["aws_access_key_id"])
        spark._jsc.hadoopConfiguration().set("fs.s3a.secret.key", aws_credentials["aws_secret_access_key"])
