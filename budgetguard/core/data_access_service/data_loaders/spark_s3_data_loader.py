...
class SparkS3DataLoader(DataLoader):
    ...
    def __build_file_path__(self, datalake_config: Dict[str, str], partition_config: Dict[str, str]):
        """
    ...
    def read(self, datalake_config: Dict[str, str], partition_config: Dict[str, str]):
        ...
        return (
            self.spark_s3_connection.read.format(datalake_config["file_extension"])
            .options(**datalake_config["options"])
            .schema(datalake_config["spark_schema"])
            .load(file_path)
        )
    
    def write(self, dataframe, datalake_config: Dict[str, str], partition_config: Dict[str, str]):
        """
