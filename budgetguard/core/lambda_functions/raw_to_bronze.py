...
def lambda_handler(event, context):
    ...
    destination_key = (
        f"{data_type}/partition_id={partition_id}/account_id={account_id}/{source_key.split('/')[-1]}"  # noqa
    )
    data = s3_read_json(source_bucket, source_key)
    data = formatter(data)
    s3_write_json(data, destination_bucket, destination_key)
    logger.info("Successfully copied data from {0} to {1}!".format(source_bucket, destination_bucket))
...
def get_ingestion_details(event: Dict) -> Tuple[str, str, str, str, str]:
    ...
    logger.info("Received event from {0} for {1}.".format(source_bucket, source_key))  # noqa
...
def s3_write_json(json_data: str, target_bucket: str, target_key: str) -> Dict:
    ...
    try:
        ...
        response = s3.put_object(Bucket=target_bucket, Key=target_key, Body=json.dumps(json_data))
...
class Formatter:
    ...
    def format_transactions(self, transactions):
        ...
        for transaction_status, transactions_info in transactions["transactions"].items():
            logger.info("Flattening transactions for status {0}...".format(transaction_status))  # noqa
            logger.info(
                "Number of transactions with status {0}: {1}".format(transaction_status, len(transactions_info))
            )
            for transaction in transactions_info:
                transaction["transactionStatus"] = transaction_status
                transaction["transactionCurrency"] = transaction["transactionAmount"]["currency"]
                transaction["transactionAmount"] = transaction["transactionAmount"]["amount"]
                if transaction.get("debtorAccount"):
                    transaction["debtorAccountIban"] = transaction["debtorAccount"]["iban"]
                    del transaction["debtorAccount"]
                else:
                    transaction["creditorAccountIban"] = transaction["creditorAccount"]["iban"]
                    del transaction["creditorAccount"]
                transaction["balanceAfterTransactionAmount"] = transaction["balanceAfterTransaction"]["balanceAmount"][
                    "amount"
                ]
                transaction["balanceAfterTransactionCurrency"] = transaction["balanceAfterTransaction"][
                    "balanceAmount"
                ]["currency"]
                del transaction["remittanceInformationUnstructured"]
                del transaction["balanceAfterTransaction"]
                transactions_flatten.append({self.camel_to_snake(k): v for k, v in transaction.items()})
    ...
    def format_balances(self, balances):
        ...
        balances_flattened = [
            {
                "balanceAmount": balance.get("balanceAmount", {}).get("amount", ""),
                "balanceCurrency": balance.get("balanceAmount", {}).get("currency", ""),
                "balanceType": balance.get("balanceType", ""),
            }
            for balance in balances_flattened
        ]
        balances_flattened = [
            {self.camel_to_snake(key): value for key, value in balance.items()} for balance in balances_flattened
        ]
        return balances_flattened

    def format_details(self, details):
        details_flattened = details.get("account", {})
        details_flattened = {self.camel_to_snake(k): v for k, v in details_flattened.items()}
